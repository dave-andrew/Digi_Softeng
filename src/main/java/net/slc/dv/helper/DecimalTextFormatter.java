...
public class DecimalTextFormatter extends TextFormatter<Number> {
    ...
    public DecimalTextFormatter(int minDecimals, int maxDecimals, double minValue, double maxValue) {
        super(
                getStringConverter(minDecimals, maxDecimals),
                0,
                getUnaryOperator(minDecimals, true, -1, minValue, maxValue));
    ...
    private static UnaryOperator<javafx.scene.control.TextFormatter.Change> getUnaryOperator(
            int maxDecimals, boolean allowsNegative, int noOfDigitsBeforeDecimal, double minValue, double maxValue) {
                ...
                if (object == null
                    ...
                    if (decPos > 0) {
                        int numberOfDecimals =
                                change.getControlNewText().substring(decPos + 1).length();
